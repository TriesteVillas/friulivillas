...
// L'apex, non il www: friulivillas.com risponde già sull'apex e mandare i
// canonical sul www costerebbe un redirect in più su ogni pagina.
...
export function pageOpenGraph(
  ...
  return {
    ...
    images: [
      image
        ? { url: image }
        : { url: "/brand/og-default.jpg", width: 1200, height: 630, alt: "FriuliVillas" },
    ],
  };
}
...
//
// NAP identico a quello di TriesteVillas (stessa srl, stessa sede, stesso
// recapito): FriuliVillas è un MARCHIO del gruppo, non una seconda sede. Tenere
// indirizzo e telefono allineati fra i siti del gruppo è ciò che impedisce a
// Google di leggerli come due attività distinte con dati in conflitto.
// `sameAs` è assente di proposito: FriuliVillas non ha profili social propri, e
// puntare a quelli di TriesteVillas dichiarerebbe un'identità che non è la sua.
export function orgJsonLd() {
  return {
    ...
    description:
      "Il marchio del gruppo TriesteVillas dedicato al Friuli Venezia Giulia: ville e proprietà selezionate fuori dal capoluogo, con la stessa regia di Trieste.",
    url: SITE_URL,
    ...
    areaServed: { "@type": "AdministrativeArea", name: "Friuli-Venezia Giulia" },
    address: {
      "@type": "PostalAddress",
...
export function listingJsonLd(p: ListingSchemaInput) {
  ...
  const accommodation: Record<string, unknown> = {
    ...
    address: {
      ...
      // Nessun ripiego su un comune "tipico": su TriesteImmobiliare il default
      // "Trieste" era quasi sempre vero, qui il perimetro è l'intera regione e
      // un comune inventato è un dato strutturato sbagliato. Meglio assente.
      ...(p.comune ? { addressLocality: p.comune } : {}),
      addressRegion: "Friuli-Venezia Giulia",
      addressCountry: "IT",
    },
    ...(p.mq
